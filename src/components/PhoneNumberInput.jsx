...
import { formatNational, getPhoneFormat } from '../lib/phoneFormat.js'
 ...
 *
 * The number itself follows the country too: the placeholder shows that
 * country's usual format, and whatever is already typed is regrouped against
 * its mask on selection (digits are preserved, only the spacing changes).
 * `formatNational` never drops or blocks a digit, so this cannot mangle input.
 */
export default function PhoneNumberInput({
  ...
  const fmt = getPhoneFormat(sel[0])
  ...
  const select = (c) => {
    onCountryChange(c[0])
    // Same digits, regrouped for the new country - so switching from +61 to
    // +44 does not leave an Australian grouping sitting next to a UK flag.
    if (value) onValueChange(formatNational(value, getPhoneFormat(c[0]).mask))
    setOpen(false)
    setQ('')
  }
  ...
  return (
    <div className={`pn-wrap ${invalid ? 'invalid' : ''}`} ref={rootRef}>
      ...
      <input
        ...
        onChange={(e) => onValueChange(formatNational(e.target.value, fmt.mask))}
        onBlur={onBlur}
        required
      />
